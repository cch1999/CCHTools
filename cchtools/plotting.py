...
def create_comparison_plot(
    ...
    fig, axs = plt.subplots(
        2, num_images, figsize=figsize, gridspec_kw={"wspace": 0, "hspace": 0.00}
    )
    ...
    if reference_img:
        for row in [0, 1]:
            ...
            axs[row, -1].set_position(
                [last_pos.x0 + 0.03, last_pos.y0, last_pos.width, last_pos.height]
            )
    ...
    for ax in axs[1]:
        ...
        ax.set_position(
            [last_pos.x0, last_pos.y0 + mol_correction, last_pos.width, last_pos.height]
        )
