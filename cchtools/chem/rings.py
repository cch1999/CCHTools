...
from cchtools.data.constants import RING_SKELETONS, RING_ATOMS

def generate_ring_systems(ring_skeletons: Dict[str, str] = RING_SKELETONS,
                          atom_types: Dict[str, str] = RING_ATOMS) -> pd.DataFrame:
    """
    ...
    for skeleton_name, skeleton_smiles in tqdm(ring_skeletons.items()):
        ...
        num_aromatic_carbons = len([atom for atom in base_mol.GetAtoms() 
            ...
            try:
                ...
                if mol and is_valid_scaffold(mol):
                    ...
                    scaffolds.append({
                        ...
                        'num_hetero': sum(1 for atom in mol.GetAtoms() 
