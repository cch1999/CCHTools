    ...
    def get_seqclust(self, pdb_code: str,
        ...
        if check_obsolete and seqclust == 'None':
            ...
            if superceded is not None:
                logging.info(f"Assigning cluster for obsolete entry via superceding: {pdb_code}->{superceded} {chain_id}")
                return self.get_seqclust(superceded, chain_id, check_obsolete=False)  # assumes chain is same in superceding entry
        if seqclust == 'None':
